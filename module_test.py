...
def _resolve_checkpoint_path(model_name):
    state_dict_path = _BASE_DIR / "pretrained_model" / f"{model_name}_pretrained_model_state_dict.pth"
    legacy_path = _BASE_DIR / "pretrained_model" / f"{model_name}_pretrained_model.pth"
    return str(state_dict_path) if state_dict_path.exists() else str(legacy_path)
...
def _extract_state_dict(checkpoint):
    if isinstance(checkpoint, dict):
        ...
        if "model" in checkpoint:
            raise RuntimeError(
                "Checkpoint contains a serialized model object, not a state_dict. "
                "Please convert it first (e.g. torch.load(path)['state_dict'])."
            )
        return checkpoint
    raise RuntimeError("Unsupported checkpoint format. Expecting a state_dict dict.")
...
def load_pretrained_weights(model, model_path, device):
    """Load state_dict checkpoint. Warns on missing/unexpected keys."""
    try:
        checkpoint = torch.load(model_path, map_location=device)
    except Exception as e:
        raise RuntimeError(f"Failed to load checkpoint: {model_path}") from e
...
def test(model_name, dataset_name, featurePath):
    ...
    config_file = str(_BASE_DIR / "config" / "config_pretrained.json")
