"""
MFN ablation: remove A<->V interaction, keep T<->A + T<->V
"""
import torch
...
class A11_MFN_without_av(nn.Module):
    def __init__(self, args):
        super(A11_MFN_without_av, self).__init__()
        ...
        # TA pair (kept)
        self.att1_TA = nn.Sequential(
            nn.Linear(2 * (self.dh_l + self.dh_a), h_att1),
            nn.ReLU(), nn.Dropout(att1_dropout),
            nn.Linear(h_att1, 2 * (self.dh_l + self.dh_a)))
        self.att2_TA = nn.Sequential(
            nn.Linear(2 * (self.dh_l + self.dh_a), h_att2),
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        for i in range(t):
            ...
            # TA pair
            pair_cStar_TA = torch.cat([new_c_l, new_c_a, prev_c_l, prev_c_a], dim=1)
            att_TA = F.softmax(self.att1_TA(pair_cStar_TA), dim=1)
            cHat_sum = cHat_sum + self.att2_TA(att_TA * pair_cStar_TA)
