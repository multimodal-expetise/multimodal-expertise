...
class A11_MFN_lav(nn.Module):
    """MFN reproduction model."""
    ...
    def forward(self, text_x, audio_x, video_x):
        ...
        n = text_x.size(1)
        t = text_x.size(0)
        self.h_l = torch.zeros(n, self.dh_l, device=text_x.device)
        self.h_a = torch.zeros(n, self.dh_a, device=text_x.device)
        self.h_v = torch.zeros(n, self.dh_v, device=text_x.device)
        self.c_l = torch.zeros(n, self.dh_l, device=text_x.device)
        self.c_a = torch.zeros(n, self.dh_a, device=text_x.device)
        self.c_v = torch.zeros(n, self.dh_v, device=text_x.device)
        self.mem = torch.zeros(n, self.mem_dim, device=text_x.device)
